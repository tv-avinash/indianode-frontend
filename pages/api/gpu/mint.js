...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!TEST_ALLOW_ANY) {
      ...
      if (!rp.ok) {
        return res.status(400).json({ error: "razorpay_fetch_failed", details: pdata });
      }
      ...
      // simple promo support like index.js (TRY / TRY10 = ₹5 off)
    ...
    const secret = process.env.GPU_ORDER_TOKEN_SECRET || process.env.ORDER_TOKEN_SECRET;
    ...
    const payload = {
      ...
      pay: payId || "test",
      iat: now,
      exp: now + 7 * 24 * 3600, // 7 days validity to redeem
    };
