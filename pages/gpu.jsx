    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl mx-4 rounded-2xl bg-white shadow-xl text-slate-900">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          ...
          <button onClick={onClose} className="rounded-lg p-2 hover:bg-gray-100" aria-label="Close">Γ£ò</button>
        </div>
...
(Invoke-WebRequest -UseBasicParsing ${url}).Content | bash`;
  ...
  return (
    <>
      <Head>
        <title>Indianode ΓÇö GPU Hosting (RTX 3090)</title>
      ...
      <SiteChrome title="3090 GPU on demand ΓÇó India & International payments" subtle>
        ...
        {String(process.env.NEXT_PUBLIC_SHOW_AKASH || "1") === "1" && (
          <div className={`${busy ? "bg-amber-400/10 border-amber-300/30" : "bg-emerald-400/10 border-emerald-300/30"} border rounded-2xl p-4 text-center mb-6`}>
            <div className="mb-3">
              {busy ? <>GPU busy. You can still deploy via Akash; weΓÇÖll queue you.</> : <>GPU available. Deploy now on Akash with our ready SDLs.</>}
            </div>
          ...
          {busy && (
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
              <div className="text-sm text-amber-800 mb-3">
                GPU is busy. You can still pay now (weΓÇÖll queue it) or join the waitlist:
              </div>
            ...
            <div key={t.key} className="bg-white/95 text-slate-900 shadow-lg rounded-2xl p-6 flex flex-col justify-between">
              <div>
                ...
                <p className="text-gray-800">
                  ...
                  Γé╣{inr(t.key)} / ${usd(t.key).toFixed(2)}
                </p>
                {promoActive && (
                ...
                <p className="text-xs text-gray-500 mt-1">(Base: Γé╣{price60[t.key]} for 60 min)</p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 mt-4">
                <button
                  ...
                  disabled={disabled}
                >
                  Pay Γé╣{inr(t.key)} ΓÇó Razorpay (INR)
                </button>
                ...
                {enablePayPal && (
                  <button
                    ...
                    disabled={disabled}
                  >
                    Pay ${usd(t.key).toFixed(2)} ΓÇó PayPal (USD)
                  </button>
                )}

                <p className="text-[11px] text-gray-500 mt-1">
                  Billed in INR via Razorpay. USD shown is an approximate amount based on todayΓÇÖs rate.
                </p>
              </div>
            </div>
          ))}
