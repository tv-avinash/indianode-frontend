...
export default function GPUOrders() {
  ...
  // ΓÇ£Price for 60 minutesΓÇ¥ base (Γé╣)
  const price60 = { whisper: 100, sd: 200, llama: 300 };

  // Promo: TRY / TRY10 => Γé╣5 off total
  ...
  // Build run.sh URL on this same site (static file, not /api)
  function getRunUrl() {
    try {
...
(Invoke-WebRequest -UseBasicParsing ${url}).Content | bash`;
  ...
  async function joinWaitlist() {
    ...
    try {
      ...
      setWlMsg("Thanks! WeΓÇÖll email you as soon as the GPU is free.");
  ...
  return (
    <>
      <Head>
        <title>Indianode ΓÇö GPU Hosting for SDLS, Whisper & LLM on RTX 3090</title>
        <meta
          name="description"
        ...
        {/* OG / Twitter */}
        <meta property="og:title" content="Indianode ΓÇö GPU Hosting on RTX 3090" />
        <meta
          property="og:description"
          content="GPU hosting for SDLS, Whisper, and LLM workloads on 24GB RTX 3090."
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://www.indianode.com/gpu" />
      ...
      <div className="min-h-screen bg-gray-50 text-gray-900">
        ...
        <header className="p-6 bg-gray-900 text-white">
          <div className="mx-auto flex max-w-5xl items-center justify-between">
            ...
            <div className="text-center text-xl font-bold">Indianode GPU Cloud</div>
            <nav className="text-sm text-white/80">
              <Link href="/compute" className="mr-3 hover:text-white">
                Compute
              </Link>
        ...
        <main className="p-8 max-w-5xl mx-auto">
          <h1 className="text-3xl font-bold mb-2 text-center">
            3090 GPU on demand ΓÇó India & International payments
          </h1>
          ...
          {SHOW_AKASH_HERO && (
            ...
            >
              <div className="mb-3">
                {busy ? (
                  <>GPU busy. You can still deploy via Akash; weΓÇÖll queue you.</>
                ) : (
            ...
            {busy && (
              <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
                <div className="text-sm text-amber-800 mb-3">
                  GPU is busy. You can still pay now (weΓÇÖll queue it) or join the
                  waitlist:
                </div>
          ...
          {/* Product cards (pay buttons only; Akash links are in the hero) */}
                  ...
                  <div>
                    ...
                    {promoActive && (
                      <p className="text-xs text-green-700 mt-1">
                        Includes promo: ΓêÆΓé╣{offInr} (Γëê${offUsd.toFixed(2)})
                      </p>
                    )}

                    <p className="text-xs text-gray-500 mt-1">
                      (Base: Γé╣{price60[t.key]} for 60 min)
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-2 mt-4">
                    ...
                    {enablePayPal && (
                      <button
                        ...
                        disabled={disabled}
                      >
                        Pay ${usd.toFixed(2)} ΓÇó PayPal (USD)
                      </button>
                    )}

                    <p className="text-[11px] text-gray-500 mt-1">
                      Billed in INR via Razorpay. USD shown is an approximate
                      amount based on todayΓÇÖs rate. Prefer Akash? Use the SDL
                      buttons at the top.
                    </p>
                  </div>
                </div>
        ...
        <section className="mt-16 border-t pt-10 pb-6 text-center text-sm text-gray-700">
          <p className="mb-2">
            ≡ƒÆ¼ Looking for custom pricing, discounts, or rate concessions? Reach
            out:
          </p>
          <p>
            Email:{" "}
      ...
      <Modal
        ...
        title="Payment verified ΓÇö run this command"
      >
